...
from attention import self_attention, multi_attention
from ner_utils import pad_sequences, get_tag

class BLSTM_CRF(object):
    def __init__(self, epoch_num, hidden_dim, embeddings, lr, vocab_words, vocab_tags,
                output_path, logger, heads, size_per_head, use_multi=False, use_doc2vec=False):
    ...
    def add_placeholders(self):
        ...
        self.doc2vec_embeddings = tf.placeholder(tf.float32, shape=[None, None], name="doc2vec_embeddings")
    ...
    def blstm_layer_op(self):
        with tf.variable_scope('blstm'):
            ...
            if self.use_multi:
                if self.use_doc2vec:
                    output = multi_attention(output, self.heads, self.size_per_head, self.sequence_lengths, self.doc2vec_embeddings, state=True)
                else:
                    output = multi_attention(output, self.heads, self.size_per_head, self.sequence_lengths)
            else:
                if self.use_doc2vec:
                    output = self_attention(output, 2*self.hidden_dim, self.sequence_lengths, self.doc2vec_embeddings, state=True)
                else:
                    output = self_attention(output, 2*self.hidden_dim, self.sequence_lengths)
            output = tf.nn.dropout(output, self.dropout_pl)

        with tf.variable_scope("proj"):
            if self.use_multi:
                parameter_shape = self.heads * self.size_per_head
            else:
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(self.epoch_num):
                ...
                for step, (seqs, labels) in enumerate(train):
                    ...
                    if self.use_doc2vec:
                        fd[self.doc2vec_embeddings] = train_doc2vec[step]
    ...
    def dev_one_epoch(self, sess, dev, mode, data_doc2vec=None):
        ...
        for step, (seqs, labels) in enumerate(dev):
            ...
            if self.use_doc2vec:
                feed_dict[self.doc2vec_embeddings] = data_doc2vec[step]
